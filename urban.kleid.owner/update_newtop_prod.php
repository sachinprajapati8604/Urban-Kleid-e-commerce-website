...
if (count($_FILES) > 0) {
  ...
  if (is_uploaded_file($_FILES['userImage']['tmp_name']) || ($_FILES['userImage2']['tmp_name']) || ($_FILES['userImage3']['tmp_name']) || ($_FILES['userImage4']['tmp_name'])) {
   ...
   $folder3 = "../img/" . $filename4;
    ...
    $sql = "UPDATE `newproducts` SET `pro_name`='$p_title',`pro_shortdetail`='$p_detail',`pro_price`='$p_price',`pro_bid_price`='$p_bidprice',`pro_imagename`='{$filename}',`imageType`='{$imageProperties['mime']}',`vp_img2`='{$filename2}',`vp_img3`='{$filename3}',`vp_img4`='{$filename4}',`created`=current_timestamp() WHERE pro_id='$proid'";
...
$proid=$_GET['proid'];
$sql = "SELECT * FROM `newproducts` where pro_id='$proid'";
...
<h2 class="text-center">UPDATE PRODUCTS </h2>
 <form action="" method="post" enctype="multipart/form-data">
   <p class="mb-3 font-weight-normal text-center">FILL THE INFORMATION BELOW:</p>
   <hr>
